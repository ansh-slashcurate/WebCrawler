...
def _format_tenders(batch):
    ...
    for record_id, record in batch:
        snippet = " | ".join(
            filter(None, [record.get("title"), record.get("reference_no"), record.get("closing_date")])
        )
...
def classify_batch(records, tags):
    ...
    input record. A batch that fails (bad response, timeout, rate limit) is
    caught per-batch and marked "error" instead of raising, so one bad LLM
    call can't lose every other record in the run."""
    ...
    for batch in _chunks(indexed, BATCH_SIZE):
        ...
        try:
            parsed: BatchResult = chain.invoke({
                "tags": _format_tags(tags),
                "tenders": _format_tenders(batch),
            })
            by_id = {m.record_id: m for m in parsed.results}
            for record_id in batch_ids:
                match = by_id.get(record_id)
                # the prompt requires exact tag names copied verbatim, but a
                # model can still hallucinate/paraphrase one - drop anything
                # that isn't a real configured tag rather than let a
                # near-miss like "AI" silently stand in for "AI RFP" and
                # break the UI's exact-match tag filter
            ...
            for record_id in batch_ids:
                results[record_id] = {"classification": "error", "matched_tags": [], "reason": str(e)[:200]}
